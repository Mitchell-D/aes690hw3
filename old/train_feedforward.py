...
def train_feedforward(X, Y, model_config, model_dir):
    """
    Train a feedforward model according to a config dictionary keys below.
    ...
    "dense_kwargs":Keyword arguments for keras Dense layers
    "node_list":List of ints representing the number of outputs per layer
    ...
    model = mm.feedforward(
            name=config.get("model_name"), ## feedforward
            node_list=config.get("node_list"),
            num_inputs=len(config.get("input_feats")),
            num_outputs=len(config.get("output_feats")),
            batchnorm=config.get("batchnorm"),
            dropout_rate=config.get("dropout_rate"),
            dense_kwargs=config.get("dense_kwargs"),
            )

    ## Create and add some information to the dedicated model directory
    model_dir = model_parent_dir.joinpath(config.get("model_name"))
    ...
    model.summary()
    summary_path = model_dir.joinpath(config.get("model_name")+"_summary.txt")
    ...
    callbacks = [
            tf.keras.callbacks.EarlyStopping(
                monitor=config.get("early_stop_metric"),
    ...
    hist = model.fit(
            gT.batch(config.get("batch_size")).prefetch(config.get("batch_buffer")),
            epochs=config.get("max_epochs"),
            ## Number of batches to draw per epoch. Use full dataset by default
            #steps_per_epoch=config.get("train_steps_per_epoch"),
            validation_data=gV.batch(config.get("batch_size")
                                     ).prefetch(config.get("batch_buffer")),
            ...
            validation_freq=config.get("val_frequency"),
            callbacks=callbacks,
            verbose=2,
            )
    ...
    model.save(model_dir.joinpath(config.get("model_name")+"_keras.hdf5"))
    ## Save the previous best-performing model from the last checkpoint
    best_model.rename(model_dir.joinpath(config.get("model_name")+"_final.hdf5"))
...
if __name__=="__main__":
    ...
    config = {
            ...
            "weighted_metrics":["mae", "mse"], ## Also recorded in prog csv
            ...
            "notes":"Basic model ; Some masking ; Only key parameters ; some dropout",
            }
